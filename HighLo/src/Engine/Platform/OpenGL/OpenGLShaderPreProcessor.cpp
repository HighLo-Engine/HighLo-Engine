...
namespace highlo
{
	...
	ShaderType OpenGLShaderPreProcessor::PreProcessHeader(HLString &contents, bool &isGuarded, std::unordered_set<HLString> &specialMacros, const std::unordered_set<IncludeData> &includeData, const FileSystemPath &fullPath)
	{
		...
		while (startOfShaderStage != std::string::npos)
		{
			...
			if (tokens[index] == "#")
			{
				...
				if (tokens[index] == "shader")
				{
					...
					GLenum foundStage = utils::ShaderStageFromString(std::string(stage.begin(), stage.end()));
		...
		return utils::GLStageToShaderType(stagesInHeader);
	}

	std::unordered_map<ShaderType, HLString> OpenGLShaderPreProcessor::PreProcessShader(const HLString &source, std::unordered_set<HLString> &specialMacros, ShaderLanguage language)
	{
		std::stringstream sourceStream;
		...
		std::unordered_map<GLenum, HLString> shaderSources;
		std::vector<std::pair<GLenum, uint64>> stagePositions;
		...
		return utils::ConvertOpenGLStageToShaderType(shaderSources);
	}
}
