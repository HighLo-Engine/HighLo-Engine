...
namespace highlo
{
	namespace utils
	{
		static VkPrimitiveTopology GetVulkanTopology(PrimitiveType topology)
		{
			switch (topology)
			{
			...
			HL_ASSERT(false, "Unknown toplogy");
		...
		static VkCompareOp GetVulkanCompareOperator(const DepthCompareOperator compareOp)
		{
			...
			HL_ASSERT(false, "Unknown Operator");
	...
	VulkanVertexArray::~VulkanVertexArray()
	{
		...
		Renderer::SubmitWithoutResources([pipeline = m_VulkanPipeline, pipelineCache = m_PipelineCache, pipelineLayout = m_PipelineLayout]()
		{
	...
	{
		Ref<VulkanVertexArray> instance = this;
		Renderer::Submit([instance]() mutable
		{
			...
			pPipelineLayoutCreateInfo.setLayoutCount = (uint32)descriptorSetLayouts.size();
			pPipelineLayoutCreateInfo.pSetLayouts = descriptorSetLayouts.data();
			pPipelineLayoutCreateInfo.pushConstantRangeCount = (uint32)vulkanPushConstantRanges.size();
			...
			inputAssemblyState.topology = utils::GetVulkanTopology(instance->m_Specification.RenderType);
			...
			if (instance->m_Specification.RenderType == PrimitiveType::Lines || instance->m_Specification.RenderType == PrimitiveType::LineStrip || instance->m_Specification.Wireframe)
			...
			depthStencilState.depthCompareOp = utils::GetVulkanCompareOperator(instance->m_Specification.DepthCompareOp);
			...
			BufferLayout &vertexLayout = instance->m_Specification.Layout;
			BufferLayout &instanceLayout = instance->m_Specification.InstanceLayout;
			BufferLayout &boneInfluenceLayout = instance->m_Specification.BoneInfluenceLayout;
			...
			utils::SetDebugUtilsObjectName(device, VK_OBJECT_TYPE_PIPELINE, instance->m_Specification.DebugName, instance->m_VulkanPipeline);
		});
	}
	...
	void VulkanVertexArray::SetUniformBuffer(const Ref<UniformBuffer> &uniformBuffer, uint32 binding, uint32 set)
	{
		Ref<VulkanVertexArray> instance = this;
