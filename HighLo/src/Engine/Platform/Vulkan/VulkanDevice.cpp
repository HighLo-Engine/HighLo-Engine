...
namespace highlo
{
	...
	VulkanDevice::VulkanDevice(const Ref<PhysicalDevice> &physicalDevice, VkPhysicalDeviceFeatures enabledFeatures)
	...
	void VulkanDevice::Init()
	{
		...
		// TODO: Do we need to enable any other extensions (eg. NV_RAYTRACING?)
	...
	Ref<VulkanCommandPool> VulkanDevice::GetThreadLocalCommandPool()
	{
		...
		HL_ASSERT(m_CommandPools.find(threadID) != m_CommandPools.end());
	...
	VulkanPhysicalDevice::VulkanPhysicalDevice()
	{
		...
		{
			HL_CORE_WARN("Could not find discrete GPU.");
			selectedPhysicalDevice = physicalDevices.back();
		}

		HL_ASSERT(selectedPhysicalDevice, "Could not find any physical devices!");
		...
		if (extCount > 0)
		{
			...
			{
				HL_CORE_TRACE("Selected physical device has {0} extensions", extensions.size());
	...
	{
		return m_SupportedExtensions.find(name) != m_SupportedExtensions.end();
	}
	...
	VulkanPhysicalDevice::QueueFamilyIndices VulkanPhysicalDevice::GetQueueFamilyIndices(int32 flags)
	{
