...
namespace highlo
{
	...
	void Scene::OnRenderRuntime(Ref<SceneRenderer> &renderer, Timestep ts)
	{
		...
		for (UUID &uuid : staticModelIds)
		{
			...
			HL_ASSERT(component);
			const Ref<StaticModel> &model = AssetManager::Get()->GetAsset<StaticModel>(component->Model);
			HL_ASSERT(model);
		...
		if (renderer->GetFinalRenderTexture())
		{
			...
			for (UUID &uuid : textIds)
			{
				...
				const Ref<Font> &font = AssetManager::Get()->GetAsset<Font>(component->FontAsset);
	...
	void Scene::SetViewportSize(uint32 width, uint32 height)
	{
		...
		// Resize all cameras
		const std::vector<UUID> &cameras = m_Registry.View<CameraComponent>();
		for (const UUID &uuid : cameras)
		{
	...
	void Scene::SortEntities()
	{
		...
		{
			const auto &lhsEntity = m_EntityIDMap.find(lhs);
			const auto &rhsEntity = m_EntityIDMap.find(rhs);
	...
	void Scene::UpdateEntity(const Entity &entity)
	{
