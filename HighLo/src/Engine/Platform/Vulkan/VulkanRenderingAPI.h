...
namespace highlo
{
	...
	public:
		...
		virtual void BeginRenderPass(
			...
			bool shouldClear = false) override;
		...
		virtual void DrawIndexed(
			...
			bool depthTest = true,
			const glm::mat4 &localTransform = glm::mat4(1.0f)) override;
		...
		virtual void DrawIndexedControlPointPatchList(
			Ref<VertexArray> &va,
			PrimitiveType type = PrimitiveType::Patch) override;
		...
		virtual Ref<Environment> CreateEnvironment(
			const FileSystemPath &filePath,
			uint32 cubemapSize = 2048,
			uint32 irradianceMapSize = 32) override;
